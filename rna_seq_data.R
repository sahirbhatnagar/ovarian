...
data2 <- foreach(i = files) %dopar% read.table.ffdf(file=i, header = TRUE)
...
k<-read.table.ffdf(file=files[1], header = TRUE)

j<-as.ffdf.data.frame(data2)

rm(x)
rstudiologs <- NULL
files <- sort(list.files(pattern = "[.]txt$")[1:5])
for(file in files){
  #print(file)
  #con <- 
  x <- read.table(file=file, header = TRUE, colClasses=c("character","character","numeric","numeric","numeric"))  
