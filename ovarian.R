...
o.pheno <- read.csv("/home/sahir/Dropbox/PhD/Year 1/Prelim Analysis Nov 2013/ovarian/TCGA_OV_PhenotypeFile.csv", 
    ...
    "TUMORSTAGE", "TUMORGRADE", "ProgressionFreeStatus", "ProgressionFreeSurvival..mos..")]
o.pheno[o.pheno == ""] <- NA
...
med.surv <- data.frame(time = c(16.6, 16.6, 16.8, 16.8), quant = c(0.5, 0, 0.5, 0), 
    TP53.class = c("TP53 missense", "TP53 missense", "TP53 null", "TP53 null"))
pl2 + geom_line(data = med.surv, aes(time, quant, group = TP53.class), col = "darkblue", 
    linetype = 3) + geom_point(data = med.surv, aes(time, quant, group = TP53.class), 
    col = "darkblue") + annotate("text", x = 14, y = 0.25, label = "median")
...
# parametric model
with(o.pheno2, survplot(survfit(pheno.obj ~ TP53.class), conf = "none", fun = qnorm, 
    logt = TRUE))

fit.aft <- psm(pheno.obj ~ AgeAtDiagnosis..yrs. + TUMORGRADE + tumorstage + TP53.class, 
    data = o.pheno2, dist = "lognormal")
